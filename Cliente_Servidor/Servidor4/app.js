...
//Socket servidor
var http = require('http').Server(express).listen(6001);
var io_server = require('socket.io')(http);
//Socket cliente
var io_client = require('socket.io-client')('http://localhost:3001'); // Socket usado para eleição


// ------- Variaveis empregadas no gossip --------------

var http2 = require('http').Server(express).listen(6003); // Porta da qual recebera a tabela de alguem
var recebe_tab_gossip = require('socket.io')(http2);

var servidores_possiveis = [1, 2, 3]; // Cada elemento eh o numero do servidor (para tornar a escolha da porta mais flexivel, caso a convencao mude)
...
setInterval(function() {
  ...
  if (tempo_local % 10 == 0) {
    ...
    envia_tab_gossip1.emit('gossip', '4', tempo_local, tab_gossip);
    envia_tab_gossip2.emit('gossip', '4', tempo_local, tab_gossip);
  }
...
var INIC_ELEICAO = false; // Variavel utilizada para testes -> indica, manualmente, qual no' ira iniciar a eleicao
...
console.log('---------------------------------------');
console.log('    Eu sou o servidor de numero 4      ');
...
io_server.on('connection', function(socket) {
  ...
  socket.on('inic_eleicao', function(port_received) {
    if (port_received == true) {
      ...
      console.log('Servidor 4 iniciou a eleicao');
      console.log('Servidor 4 enviou id = ' + id_forward + ' para o servidor de numero 1');
    }
  });
  ...
  socket.on('election', function(id_received) {
    if (id == id_received) {
      ...
      console.log('Servidor 4 comecou o elected');
    } else {
      if (id > id_received) {
        id_forward = id;
      } else {
    ...
    console.log('Servidor 4 recebeu id = ' + id_received + ' do servidor de numero 3');
    console.log('Servidor 4 enviou id = ' + id_forward + ' para o servidor de numero 1');
  ...
  socket.on('id_elected', function(id_elected) {
    if (id == id_elected) {
      ...
      var io_client2 = require('socket.io-client')('http://localhost:7001');
      io_client2.emit('elected', 'localhost', '6000');
    } else {
      elected = id_elected;
      io_client.emit('id_elected', elected);
...
io_client.on('connect', function() {
  console.log("socket connected with server 3001");
});
